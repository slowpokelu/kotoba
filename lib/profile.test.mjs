...
import { allAnswers, allValid, answersFor } from './word-pools.mjs';
import {
  newGame,
  dailyAnswer,
  outcome,
...
test('multi-length backups preserve old daily and practice saves and reject mismatched records', () => {
  const data = {
    app: 'kotoba',
    version: 1,
    theme: 'system',
    ...
    games: [
      ...
      ...[3, 5, 6].map((n) =>
        newGame(answersFor(n)[0].reading, `practice:${n}`),
      ),
    ],
  };
  assert.deepEqual(
    parseBackup(JSON.stringify(data), allAnswers, allValid),
    data,
  );
  const old = {
    app: 'kotoba',
  ...
  assert.deepEqual(parseBackup(JSON.stringify(old), allAnswers, allValid), old);
  for (const bad of [
    { ...data, practiceLength: 2 },
    { ...data, practiceLength: '6' },
    { ...data, games: [newGame('がっこう', 'practice:6')] },
    { ...data, games: [newGame('さくら', 'practice')] },
    { ...data, games: [newGame('さくら', 'practice:7')] },
    {
      ...data,
      games: [{ ...newGame('さくら', 'practice:3'), guesses: ['がっこう'] }],
    },
  ])
    assert.throws(() => parseBackup(JSON.stringify(bad), allAnswers, allValid));
});
const valid = new Set(
